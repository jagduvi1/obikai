...
# ─────────────────────────────────────────────────────────────────────────────
# Obikai web-member PWA image — build the Vite SPA, serve the static bundle with Caddy.
#
#   docker build -f docker/web-member.Dockerfile -t obikai-web-member .
#
...
# Build web-member and its workspace deps (turbo resolves the order); then render the Caddyfile.
RUN pnpm exec turbo run build --filter=@obikai/web-member
...
COPY --from=build /repo/apps/web-member/dist /srv
